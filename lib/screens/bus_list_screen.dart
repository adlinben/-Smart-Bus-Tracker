...
class _BusListScreenState extends State<BusListScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final waitingBuses = buses
        .where((bus) => bus.status.toUpperCase().contains("WAITING")).toList();
    ...
    return Scaffold(
      ...
      body: Column(crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          Center(
            child: Text("${widget.source} → ${widget.destination}",
          ...
          Expanded(
            child: ListView(
              children: [
                ...
                if (waitingBuses.isNotEmpty) ...[
                  const Padding(padding: EdgeInsets.symmetric(
                      horizontal: 16,
                      vertical: 8,
                    ),
                    child: Text("Waiting Buses",
                      style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.orange,),),
                  ),
                  ...waitingBuses.map((bus) => BusCard(bus: bus)),
                ],
                ...
                if (runningBuses.isEmpty && waitingBuses.isEmpty && scheduledBuses.isEmpty)
                  const Center(
