...
class _BusListScreenState extends State<BusListScreen> {
  ...
  List<Bus> _sortBuses(List<Bus> source) {
    ...
    sorted.sort((a, b) {
      final aTime = _timeToMinutes(
        a.busArrivalTimeAtBoardingStop,
      );
      final bTime = _timeToMinutes(
  ...
  Bus? _findRecommendedBus(List<Bus> buses) {
    ...
    final suitableBuses = buses.where((bus) {
      final arrivalMinutes = _timeToMinutes(
    ...
    suitableBuses.sort((a, b) {
      final aArrival = _timeToMinutes(
        a.busArrivalTimeAtBoardingStop,
      );
      final bArrival = _timeToMinutes(
        b.busArrivalTimeAtBoardingStop,
      );
  ...
  Widget _buildLastUpdated() {
    final bool hasUpdated = lastUpdated != null;
  ...
  Widget _buildContent(List<Bus> displayedBuses) {
    if (isLoading) {
      return const Center(
        child: CircularProgressIndicator(
          color: AppTheme.primaryRed,
    ...
    if (errorMessage != null && buses.isEmpty) {
  ...
  Widget _buildEmptyState() {
    ...
    if (selectedTab == 0) {
      ...
      subtitle =
      "No suitable upcoming buses were found for your selected time.";
      icon = Icons.schedule_rounded;
    }
