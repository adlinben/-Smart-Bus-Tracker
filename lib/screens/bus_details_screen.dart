...
class _BusDetailsScreenState extends State<BusDetailsScreen> {
  ...
  void initState() {
    ...
    _timer = Timer.periodic(
      const Duration(seconds: 30), (_) => _loadBus(),
    );
  }
  Future<void> _loadBus() async {
    try {
      ...
      final updatedBus = buses.firstWhere((b) => b.busId == bus.busId, orElse: () => bus,);
      if (!mounted) return;
      setState(() {
        bus = updatedBus;
      });
      ...
      body: RefreshIndicator(
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              _buildBusHeader(
                statusColor: statusColor, statusBackground: statusBackground, displayStatus: displayStatus,
              ),
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  color: statusBackground, borderRadius: BorderRadius.circular(14),
                ),
                child: Icon(
                  Icons.directions_bus_rounded, size: 29, color: statusColor,
                ),
              ),
          ...
          Row(
            children: [
              ...
              const Padding(
                ...
                child: Icon(
                  Icons.arrow_forward_rounded, size: 19, color: AppTheme.primaryRed,
                ),
              ),
              Expanded(
                child: _routePoint(
                  label: "DESTINATION", value: bus.destinationStop, alignRight: true,
                ),
              ),
            ],
          ),
    ...
    return Column(
      ...
      children: [
        Text(
          label,
          style: const TextStyle(
            fontSize: 9, fontWeight: FontWeight.w700, letterSpacing: 0.5, color: AppTheme.textMuted,
          ),
        ),
  ...
  Widget _section({
    ...
    return Container(
      width: double.infinity,
      color: Colors.white, padding: const EdgeInsets.fromLTRB(16, 16, 16, 18,
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
      ...
      child: ElevatedButton.icon(
        ...
        onPressed: () {
          Navigator.push(context,
            MaterialPageRoute(builder: (_) => MapScreen(bus: bus),
            ),
          );
        },
      ),
